...
public:
    ...
    /**@brief It expands the custom network representation with additional network zones provided in multiple configuration files
     *
     * It reads each configuration file into a Config object, which is then used to read and add node and connection configuration.
     * @return -1 if any exception is caught during the execution.
     */
    int expandNetworkDomain();
    /**@brief It joins the custom network zones representations into a single network domain representation by combining the connections of the joint nodes
     *
     */
    void joinZones();
