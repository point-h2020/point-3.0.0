 ...
 * Upgraded by Mays AL-Naday 2015-2018
...
void GraphRepresentation::buildIGraphTopology() {
    ...
    for (size_t i = 0; i < dm->network_nodes.size(); i++) {
        ...
        for (size_t j = 0; j < nn->connections.size(); j++) {
	      ...
	      igraph_cattribute_EAS_set(&igraph, "LID", igraph_ecount(&igraph) - 1,
					nc->LID.to_string().c_str());
		igraph_cattribute_EAS_set(&igraph, "NAME", igraph_ecount(&igraph) - 1,
										  nc->name.c_str());
				cout << "ConnectionName: " << nc->name << endl;
				
	      printf("node %s -> node %s ",
		     igraph_cattribute_VAS(&igraph, "NODEID", source_vertex_id),
		     igraph_cattribute_VAS(&igraph, "NODEID",
					   destination_vertex_id));
	      printf("link: %s \n", nc->LID.to_string().c_str());
                cout<<dm->lid2ipv6(nc->LID.to_string())<<endl;
    ...
    for (size_t i = 0; i < dm->network_nodes.size(); i++) {
		...
		igraph_cattribute_VAS_set(&igraph, "NAME", vertex_index, nn->name.c_str());
        if (dm->overlay_mode.compare("mac_ml") == 0) {
...
//      cout << "Creating RVFIDs and TMFIDs for single layer topology..." << endl;
    ...
    igraph_get_shortest_paths(&igraph, &res, NULL, from, vs, IGRAPH_OUT,NULL,NULL);
...
void GraphRepresentation::calculateRVFIDs() {
    ...
    for (size_t i = 0; i < dm->network_nodes.size(); i++) {
        ...
        nn->FID_from_RV = calculateFID(RVLabel, nn->label);
    }
}
