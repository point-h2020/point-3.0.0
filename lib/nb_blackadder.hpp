...
public:
     ...
     * @param str_opt a bucket of bytes that are strategy specific. When the IMPLICIT_RENDEZVOUS strategy is used this bucket contains a LIPSIN identifier.
     * @param str_opt_len the size of the provided bucket of bytes. When the IMPLICIT_RENDEZVOUS strategy is used str_opt_len should be FID_LEN.
     * @param a_data a bucket of data that is published.
    ...
    /**@brief this method will send a PUBLISH_DATA_iMULTICAST request to Blackadder. <b>It
     * won't block. Instead the request buffer will be put in a queue and the
     * selector thread will be notified to send the request to Blackadder.</b>
     *
     * The data bucket passed to this function is freed within the library. It
     * should be allocated with, e.g., malloc() or calloc() and it must not be
     * freed by the application after it has been published.
     *
     * @param id the full identifier of the information item for which data is
     * published.
     * @param strategy the dissemination strategy assigned to the request.
     * @param str_opt a bucket of bytes that are strategy specific. When the
     * IMPLICIT_RENDEZVOUS strategy is used this bucket contains a LIPSIN
     * identifier.
     * @param str_opt_len the size of the provided bucket of bytes. When the
     * IMPLICIT_RENDEZVOUS strategy is used str_opt_len should be FID_LEN.
     * @param nodeIds the list of node identifiers of implicit subscribers for
     * which blackadder should create a multicast FID
     * @param nodeIds List of NodeIDs which should receive the payload given in
     * a_data
     * @param a_data a bucket of data that is published.
     * @param data_len the size of the published data.
     *
     * @return Boolean indicating whether or not the message was successfully
     * sent off to the BA PID
     */
    bool publish_data(const string &id, unsigned char strategy, void *str_opt,
                      unsigned int str_opt_len, std::list<string> &nodeIds, void *a_data,
                      unsigned int data_len);
    /**@brief this method will send a PUBLISH_DATA_iSUB request to Blackadder. <b>It
     * won't block. Instead the request buffer will be put in a queue and the
     * selector thread will be notified to send the request to Blackadder.</b>
     *
     * The data bucket passed to this function is freed within the library. It
     ...
     * @param str_opt_len the size of the provided bucket of bytes. When the
     * IMPLICIT_RENDEZVOUS strategy is used str_opt_len should be FID_LEN.
     * @param isubID the full information identifier for which an implicit subscription
     * is made
     * @param a_data a bucket of data that is published.
     * @param data_len the size of the published data.
     *
     ...
     * sent off to the BA PID
     */
    bool publish_data_isub(const string &id, unsigned char strategy, void *str_opt, unsigned int str_opt_len, const string &isubID, void *a_data, unsigned int data_len);
     ...
     *
     * @return Boolean indicating whether disconnection message was successfully
     * sent off to Blackadder
     */
    bool disconnect();//SR_iSUB
