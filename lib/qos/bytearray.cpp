...
void 
ByteArray::append(uint8_t * ext_data, uint64_t ext_size){
  uint64_t old_size = size;
  extend(ext_size);
  setBytes(ext_data, ext_size, old_size,0);
}
...
/**
 * If this ends up being call many times for lot of data PRODUCES ERRORs to the RcvbufErrors: 
 * of UDP. This is cause it is time consuming _probably_... Could be done on a thread other
 * than the main socket one... 
 *
 * Anyway though calling this function intensively will add huge processing delay...
 */
void 
...
ByteArray::setBits(uint64_t b, uint8_t lownum, uint64_t offset_bytes, uint8_t offsetbits){
  ...
  uint8_t to=ceil((double)lownum/8);
  uint8_t from=(uint8_t)( 8 - ceil((double)lownum/8));
...
ByteArray::setBytes(byte * b, uint32_t num, uint64_t offset_bytes, uint8_t offsetbits){
  ...
  for (uint32_t i=0; i<num; i++){
    if (offsetbits==0) data[offset_bytes]=b[i];
    else setBits(b[i], 8, offset_bytes, offsetbits);
...
// --------------------------------------------------
void 
ByteArray::setBytes(const string &str, uint64_t offset_bytes, uint8_t offsetbits){
...
string 
ByteArray::getStrBytes(uint8_t num, uint64_t offset_bytes, uint8_t offsetbits){
  if (num+offset_bytes > size) return "";
  string res="";
...
//     result.data[size_old+i]=ba.data[i];
  memcpy(result.data+size_old, ba.data, size);
