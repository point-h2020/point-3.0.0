...
#ifndef MOLY_PRIMITIVES_PACKETDROPRATE_HH_
#define MOLY_PRIMITIVES_PACKETDROPRATE_HH_
...
/*!
 * \brief Implementation of the PACKET_DROP_RATE primitive
 */
class PacketDropRate {
public:
	/*!
	 * \brief Constructor for processes
	 */
	PacketDropRate(uint32_t nodeId, uint16_t portId, uint32_t rate);
	/*!
	 * \brief Constructor for MONAs
	 */
	PacketDropRate(uint8_t* pointer, size_t size);
	/*!
	 * \brief Destructor
	 */
	~PacketDropRate();
	...
	uint16_t portId();
	/*!
	 * \brief Print out the content of the PACKET_DROP_RATE primitive
	 *
	 * The print out order is determined by the MOLY specification
	 */
	string print();
	/*!
	 * \brief Obtain the packet drop rate stored in this class instance
	 */
	uint32_t rate();
	/*!
	 * \brief Size
...
private:
	...
	uint32_t _rate;/*!< packet drop rate */
...
#endif /* MOLY_PRIMITIVES_PACKETDROPRATE_HH_ */
