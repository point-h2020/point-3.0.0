...
namespace ns3 {
    ...
    m_pid_generator = CreateObject<UniformRandomVariable> ();  //Variable Distribution Class (UniformRandomVariable) replaces 
    double min=0;                                              //(UniformVariable) with new internal behaviour from NS3.15 and above.
    double max=65536;                                          // ****
    m_pid_generator->SetAttribute ("Min", DoubleValue(min));   // ****
    m_pid_generator->SetAttribute ("Max", DoubleValue(max));   // ****    
    }
    
    ServiceModel::~ServiceModel() {
    }

    void ServiceModel::DoInitialize(void) {        //Object and subclasses DoStart has been renamed to DoInitialize in NS3.17 and above.
    ...
    uint32_t ServiceModel::RegisterApplication(Ptr<Application> app, EventListener el) {
        ...
        if (m_eventListeners.find(app) != m_eventListeners.end()) {
            ...
            do {
            pid = m_pid_generator->GetValue (); //New behaviour for (UniformRandomVariable) variable distribution class from NS3.15 and above. 
