...
int TMIgraph::readTopology(const char *file_name) {
    ...
    for (int i = 0; i < igraph_vcount(&graph); i++) {
        ...
        ilid = Create<BitVector > (iLID);
        nodeID_iLID.insert(pair<string, Ptr<BitVector> >(nID, ilid));
        vertex_iLID.insert(pair<int, Ptr<BitVector> >(i, ilid));
    ...
    for (int i = 0; i < igraph_ecount(&graph); i++) {
        ...
        lid = Create<BitVector > (LID);
        edge_LID.insert(pair<int, Ptr<BitVector> >(i, lid));
        //cout << "edge " << i << " has LID  " << lid->to_string() << endl;
    }
    return ret;
}

Ptr<BitVector> TMIgraph::calculateFID(string &source, string &destination) {
    int vertex_id;
    Ptr<BitVector> result = Create<BitVector > (FID_LEN * 8);
    igraph_vs_t vs;
...
void TMIgraph::calculateFID(set<string> &publishers, set<string> &subscribers, map<string, Ptr<BitVector> > &result) {
    ...
    for (publishers_it = publishers.begin(); publishers_it != publishers.end(); publishers_it++) {
        ...
        result.insert(pair<string, Ptr<BitVector> >(publ, NULL));
    }
    for (subscribers_it = subscribers.begin(); subscribers_it != subscribers.end(); subscribers_it++) {
        ...
        if ((*result.find(bestPublisher)).second == NULL) {
            ...
            result[bestPublisher] = Create<BitVector > (bestFID);
        } else {
            //cout << "/*update the FID for the publisher*/" << endl;
            Ptr<BitVector> existingFID = (*result.find(bestPublisher)).second;
            /*or the result FID*/
