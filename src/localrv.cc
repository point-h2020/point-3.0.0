...
int LocalRV::initialize(ErrorHandler */*errh*/) {
    ...
    WritablePacket *p_uc = Packet::make(100);
    ...
    output(0).push(p);
    /*I will also send a subscription (IMPLICIT_RENDEZVOUS) for recieving notifications of uni-cast *-over-icn*/
    memcpy(p_uc->data(), &type, sizeof (type));
    memcpy(p_uc->data() + sizeof (type), &id_len, sizeof (id_len));
    memcpy(p_uc->data() + sizeof (type) + sizeof (id_len), gc->RVScopeUC.c_str(), gc->RVScopeUC.length());
    memcpy(p_uc->data() + sizeof (type) + sizeof (id_len) + gc->RVScopeUC.length(), /*id_len again for the prefix_id of gc->RVScope*/ &id_len, sizeof (id_len));
    memcpy(p_uc->data() + sizeof (type) + sizeof (id_len) + gc->RVScopeUC.length() + sizeof (id_len), gc->RVScope.c_str(), gc->RVScope.length());
    memcpy(p_uc->data() + sizeof (type) + sizeof (id_len) + gc->RVScopeUC.length() + sizeof (id_len) + gc->RVScope.length(), &strategy, sizeof (strategy));
    output(0).push(p_uc);
    known_rootscopes.find_insert(gc->IP_ROOTSCOPE);
    known_rootscopes.find_insert(gc->HTTP_ROOTSCOPE);
    rootScope_namesapce_Index.find_insert(gc->IP_ROOTSCOPE, NAMESPACE_IP);
    rootScope_namesapce_Index.find_insert(gc->HTTP_ROOTSCOPE, NAMESPACE_HTTP);
...
void LocalRV::push(int in_port, Packet * p) {
    ...
    String IDOfAPIEvent;
    String ID, prefixID;
    String nodeID, iSubNodeID;
    StringSet isubscribers;
    ...
    if (in_port == 0) {
        ...
        if (typeOfAPIEvent == PUBLISHED_DATA) {
            ...
            switch (type) {
                    ...
                    publish_info(_remotehost, ID, prefixID, strategy, false);
                    break;
                    ...
                    unpublish_scope(_remotehost, ID, prefixID, strategy, false);
                    break;
                    case UNPUBLISH_INFO:
                    ...
                    unpublish_info(_remotehost, ID, prefixID, strategy, false);
                    break;
                    ...
                    subscribe_scope(_remotehost, ID, prefixID, strategy, false);
                    break;
                    case SUBSCRIBE_INFO:
                    ...
                    subscribe_info(_remotehost, ID, prefixID, strategy, false);
                    break;
                    ...
                    unsubscribe_scope(_remotehost, ID, prefixID, strategy, true);
                    break;
                    ...
                    unsubscribe_info(_remotehost, ID, prefixID, strategy,true);
                    break;
                default:
            ...
            p->kill();
        } else if (typeOfAPIEvent == PUBLISHED_DATA_iSUB){
            click_chatter("LocalRV: Request for implicit Subscription, will request domain TM assistance");
            numberOfiSubscribers = *(p->data() + sizeof (typeOfAPIEvent) + sizeof (IDLengthOfAPIEvent) + (int) IDLengthOfAPIEvent * PURSUIT_ID_LEN);
            for (int i = 0; i < (int) numberOfiSubscribers; i++) {
                iSubNodeID = String((const char *)(p->data() + sizeof (typeOfAPIEvent) + sizeof (IDLengthOfAPIEvent) + IDLengthOfAPIEvent * PURSUIT_ID_LEN + sizeof (numberOfiSubscribers) + index), NODEID_LEN);
                click_chatter("LocalRV: iSubscriber: %s", iSubNodeID.quoted_hex().c_str());
                isubscribers.find_insert(StringSetItem(iSubNodeID));
                index += NODEID_LEN;
            }
            requestTMAssistanceForiSubscription(isubscribers, false);
        }
        else {
            click_chatter("LocalRV: FATAL - I am expecting only PUBLISHED_DATA pub/sub events, I got event %x", typeOfAPIEvent);
        }
    } else {
...
unsigned int LocalRV::publish_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    if ((prefixID.length() > 0) && (ID.length() == PURSUIT_ID_LEN)) {
        ret = advertise_info(_publisher, ID, prefixID, strategy, update_only);
    } else if ((prefixID.length() > 0) && (ID.length() > PURSUIT_ID_LEN)) {
        ret = readvertise_info(_publisher, ID, prefixID, strategy, update_only);
    } else {
        ret = WRONG_IDS;
...
unsigned int LocalRV::advertise_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    InformationItem *pub;
    Scope *fatherScope;
    /*rootscope identifiers determine whether the request is pure ICN or *-over-ICN*/
    unsigned char _RootScopeid;
    ...
    if (fatherScope != scopeIndex.default_value()) {
        ...
        /*InformationItem with specific ID does not exist yet*/
            ...
            if (scopeIndex.find(fullID) == scopeIndex.end()) {
                if (fatherScope->strategy == strategy) {
                    ...
                    if (pub->updatePublishers(fullID, _publisher)) {
                        ...
                        fatherScope->getSubscribers(subscribers);
                        _RootScopeid = pub->getRootScopes(&rootScope_namesapce_Index);
                        /*check if the first root scope is a *-over-icn known scope - multiple all *-over-icn scopes is a possibility but mutliple scopes of mixed POINT and random scopes is prohibited and the NAP enforce this restriction*/
                        switch (_RootScopeid) {
                            case NAMESPACE_HTTP:
                                if (subscribers.size() <= 0) {
                                    getHTTPgw(subscribers);
                                    click_chatter("LocalRV: number of HTTP GWs: %x", subscribers.size());
                                }
                            case NAMESPACE_IP:
                                click_chatter("LocalRV: IP/HTTP Root Scope of Information Item is %x", _RootScopeid);
                                rendezvous(pub, subscribers,update_only);
                                if (subscribers.size() > 0) {
                                    /*Get the publishers in order to be unpublished after a rendezvous() occured*/
                                    RemoteHostSet publishers;
                                    pub->getPublishers(publishers);
                                    for (RemoteHostSetIter pub_it = publishers.begin(); pub_it != publishers.end(); pub_it++) {
                                        /*publishers in POINT do one-shot publication as they are removed after match pub/sub occured*/
                                        unpublish_info((*pub_it)._rhpointer, ID, prefixID, strategy, update_only);
                                    }
                                }
                                break;
                            case NAMESPACE_UNKNOWN:
                                click_chatter("LocalRV: ICN Root Scope of Information Item is %x", _RootScopeid);
                                rendezvous(pub, subscribers,update_only);
                                break;
                            default:
                                break;
...
unsigned int LocalRV::readvertise_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
                ...
                if (equivalentPub == pubIndex.default_value()) {
                    if (fatherScope->strategy == strategy) {
                        ...
                        if (existingPub->updatePublishers(fullID, _publisher)) {
                            ...
                            /*check if the first root scope is a *-over-icn known scope - multiple all-POINT scopes is a possibility but mutliple scopes of mixed POINT and random scopes is prohibited and the NAP enforce this restriction*/
                            _RootScopeid = existingPub->getRootScopes(&rootScope_namesapce_Index);
                            switch (_RootScopeid) {
                                case NAMESPACE_HTTP:
                                    if (subscribers.size() <= 0) {
                                        getHTTPgw(subscribers);
                                ...
                                case NAMESPACE_IP:
                                    click_chatter("LocalRV: IP/HTTP Root Scope of Information Item is %x", _RootScopeid);
                                    rendezvous(existingPub, subscribers,update_only);
                                    if (subscribers.size() > 0) {
                                        /*Get the publishers in order to be unpublished after a rendezvous() occured*/
                                        RemoteHostSet publishers;
                                        existingPub->getPublishers(publishers);
                                        for (RemoteHostSetIter pub_it = publishers.begin(); pub_it != publishers.end(); pub_it++) {
                                            /*publishers in POINT do one-shot publication as they are removed after match pub/sub occured*/
                                            unpublish_info((*pub_it)._rhpointer, suffixID, prefixID, strategy, update_only);
                                        }
                                    }
                                    break;
                                case NAMESPACE_UNKNOWN:
                    ...
                    if (fatherScope->strategy == strategy) {
                        if (equivalentPub->updatePublishers(fullID, _publisher)) {
                            ...
                            /*check if the first root scope is a *-over-icn known scope - multiple all-POINT scopes is a possibility but mutliple scopes of mixed POINT and random scopes is prohibited and the NAP enforce this restriction*/
                            _RootScopeid = equivalentPub->getRootScopes(&rootScope_namesapce_Index);
                            switch (_RootScopeid) {
                                case NAMESPACE_HTTP:
                                    if (subscribers.size() <= 0) {
                                        getHTTPgw(subscribers);
                                ...
                                case NAMESPACE_IP:
                                    click_chatter("LocalRV: IP/HTTP Root Scope of Information Item is %x", _RootScopeid);
                                    rendezvous(equivalentPub, subscribers,update_only);
                                    if (subscribers.size() > 0) {
                                        /*Get the publishers in order to be unpublished after a rendezvous() occured*/
                                        RemoteHostSet publishers;
                                        equivalentPub->getPublishers(publishers);
                                        for (RemoteHostSetIter pub_it = publishers.begin(); pub_it != publishers.end(); pub_it++) {
                                            /*publishers in POINT do one-shot publication as they are removed after match pub/sub occured*/
                                            unpublish_info((*pub_it)._rhpointer, suffixID, prefixID, strategy, update_only);
                                        }
...
unsigned int LocalRV::unpublish_scope(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    Scope *sc;
    Scope *fatherScope;
    String fullID = prefixID + ID;
            ...
            if (fatherScope != scopeIndex.default_value()) {
                ...
                for (InformationItemSetIter pub_it = sc->informationitems.begin(); pub_it != sc->informationitems.end(); pub_it++) {
                    ...
                    for (IdsHashMapIter it = sc->ids.begin(); it != sc->ids.end(); it++) {
                        ...
                        unpublish_info(_publisher, pubSuffixID, scPrefixID, strategy, update_only);
                    }
                }
...
unsigned int LocalRV::unpublish_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    unsigned int known_scope;
    InformationItem *pub;
    Scope *fatherScope;
    StringSet _RootScopeids;
    String fullID = prefixID + ID;
    ...
    if (pub != pubIndex.default_value()) {
        if (fatherScope->strategy == strategy) {
			pub->getRootScopes(_RootScopeids);
            ...
            if (pair->first.find(_publisher) != pair->first.end()) {
                ...
                if (pub->fatherScopes.size() == 1) {
                    if (!pub->checkForOtherPubSub(fatherScope)) {
                        ...
                        /*do the rendezvous again, only if the RootScope is not a known scope in the range reserved for *-over-ICN abstractions*/
                        known_scope = (known_rootscopes.find((*_RootScopeids.begin())._strData) != known_rootscopes.end())? 1 : 0;
                        switch (known_scope) {
                            case 1:
                                click_chatter("LocalRV: Do not perform Rendevous again after unpublish for unicast-dissemination");
                                break;
                            case 0:
                            {
                                RemoteHostSet subscribers;
...
unsigned int LocalRV::subscribe_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    if ((prefixID.length() == 0) && (ID.length() == PURSUIT_ID_LEN)) {
        ret = subscribe_root_scope(_subscriber, ID, strategy, update_only);
    } else if ((prefixID.length() > 0) && (ID.length() == PURSUIT_ID_LEN)) {
        ret = subscribe_inner_scope(_subscriber, ID, prefixID, strategy, update_only);
    } else {
        ret = WRONG_IDS;
...
unsigned int LocalRV::subscribe_root_scope(RemoteHost *_subscriber, String &ID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    unsigned int known_scope;
    Scope *sc = NULL;
        ...
        if (sc->strategy == strategy) {
            if (sc->updateSubscribers(ID, _subscriber)) {
                ...
                for (pub_it = _informationitems.begin(); pub_it != _informationitems.end(); pub_it++) {
                    ...
                    /*carefull, the below assumes that all publishers of the information item have either: 1- published it under known father scopes for *-over-icn or, 2- published under normal ICN scopes */
                    known_scope = (known_rootscopes.find(ID) != known_rootscopes.end())? 1 : 0;
                    switch (known_scope) {
                        case 1:
                            /*A *-over-icn known scope, so it is counted as uni-cast ICN pub/sub that follows one-shot semantics*/
                        {
                            click_chatter("LocalRV: IP/HTTP Root Scope of Information Item is %s", ID.quoted_hex().c_str());
                            rendezvous((*pub_it)._iipointer, subscribers,update_only);
                            RemoteHostSet publishers;
                            (*pub_it)._iipointer->getPublishers(publishers);
                            /*publishers in POINT do one-shot publication as they are removed after match pub/sub occured*/
                            for (RemoteHostSetIter it = publishers.begin(); it != publishers.end(); it++) {
                                /*get the identifier of the information item, here we only look at the first full identifier*/
                                /*because it does not matter if there are multiple identifiers, they will all should end with the same information identifier in the last PURSUIT_ID_LEN*/
                                IID = (*(*pub_it)._iipointer->ids.begin()).first;
                                suffixID = IID.substring(IID.length() - PURSUIT_ID_LEN, PURSUIT_ID_LEN) ;
                                unpublish_info((*it)._rhpointer, suffixID, ID, strategy, update_only);
                            }
                        }
                            break;
                        case 0:
                            /*Not a *-over-icn known scope, so it is counted as PURE ICN pub/sub that follows channel semantics*/
                        {
                            click_chatter("LocalRV: ICN Root Scope of Information Item is %s", ID.quoted_hex().c_str());
                            rendezvous((*pub_it)._iipointer, subscribers,update_only);
                        }
                            break;
                        default:
...
unsigned int LocalRV::subscribe_inner_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    unsigned int known_scope;
    Scope *sc;
    Scope *fatherScope;
                ...
                if (sc->strategy == strategy) {
                    if (sc->updateSubscribers(fullID, _subscriber)) {
                        ...
                        for (pub_it = _informationitems.begin(); pub_it != _informationitems.end(); pub_it++) {
                            ...
                            (*pub_it)._iipointer->getRootScopes(_RootScopeids);
                            known_scope = (known_rootscopes.find((*_RootScopeids.begin())._strData) != known_rootscopes.end())? 1 : 0;
                            switch (known_scope) {
                                case 1:
                                    /*A *-over-icn known scope, so it is counted as uni-cast ICN pub/sub that follows one-shot semantics*/
                                    /*notice branckets here are required to localize the declaration of variables within the case and not propagate to subsequent cases*/
                                {
                                    click_chatter("LocalRV: IP/HTTP Root Scope of Information Item is %s", (*_RootScopeids.begin())._strData.quoted_hex().c_str());
                                    rendezvous((*pub_it)._iipointer, subscribers,update_only);
                                    RemoteHostSet publishers;
                                    (*pub_it)._iipointer->getPublishers(publishers);
                                    /*publishers in POINT do one-shot publication as they are removed after match pub/sub occured*/
                                    for (RemoteHostSetIter it = publishers.begin(); it != publishers.end(); it++) {
                                        ...
                                        IID = (*(*pub_it)._iipointer->ids.begin()).first;
                                        suffixID = IID.substring(IID.length() - PURSUIT_ID_LEN, PURSUIT_ID_LEN) ;
                                        unpublish_info((*it)._rhpointer, suffixID, fullID, strategy, update_only);
                                    }
                                }
                                    break;
                                case 0:
                                    /*Not a *-over-icn known scope, so it is counted as PURE ICN pub/sub that follows channel semantics*/
                                {
                                    click_chatter("LocalRV: ICN Root Scope of Information Item is %s", (*_RootScopeids.begin())._strData.quoted_hex().c_str());
                                    rendezvous((*pub_it)._iipointer, subscribers,update_only);
                                }
                                default:
                                    break;
...
unsigned int LocalRV::subscribe_info(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    unsigned int known_scope;
    InformationItem *pub;
    Scope *fatherScope;
                    ...
                    if (pub->updateSubscribers(fullID, _subscriber)) {
                        ...
                        switch (known_scope) {
                                ...
                                /*notice branckets here are required to localize the declaration of variables within the case and not propagate to subsequent cases*/
                            {
                                ...
                                /*publishers in POINT do one-shot publication as they are removed after match pub/sub occured*/
                                for (RemoteHostSetIter it = publishers.begin(); it != publishers.end(); it++) {
                                    unpublish_info((*it)._rhpointer, ID, prefixID, strategy, update_only);
                                }
                            }
                                break;
                            case 0:
...
unsigned int LocalRV::unsubscribe_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    unsigned int known_scope;
    Scope *sc;
    Scope *fatherScope;
    String _RootScopeid;
    _RootScopeid = prefixID.substring(0, PURSUIT_ID_LEN);
    String fullID = prefixID + ID;
    sc = scopeIndex.get(fullID);
    if (sc != scopeIndex.default_value()) {
        if (sc->strategy == strategy) {
            ...
            if (fatherScope != scopeIndex.default_value()) {
                ...
                if (pair->second.find(_subscriber) != pair->second.end()) {
                    ...
                    InformationItemSet _informationitems;
                    sc->getInformationItems(_informationitems);
                    /*then, for each one do the rendez-vous process, only if the RootScope is not a known scope in the range reserved for *-over-ICN abstractions*/
                    known_scope = (known_rootscopes.find(_RootScopeid) != known_rootscopes.end())? 1 : 0;
                    switch (known_scope) {
                        case 1:
                        {
                            click_chatter("LocalRV: Do not perform Rendevous again after unsubscribe for unicast-dissemination");
                        }
                            break;
                        case 0:
                        {
                            for (InformationItemSetIter pub_it = _informationitems.begin(); pub_it != _informationitems.end(); pub_it++) {
                                click_chatter("LocalRV: ICN Root Scope of Information Item is %s", _RootScopeid.quoted_hex().c_str());
                                RemoteHostSet subscribers;
                                (*pub_it)._iipointer->getSubscribers(subscribers);
                                /*careful here...I have multiple fathers*/
                                for (ScopeSetIter fathersc_it = (*pub_it)._iipointer->fatherScopes.begin(); fathersc_it != (*pub_it)._iipointer->fatherScopes.end(); fathersc_it++) {
                ...
                if (pair->second.find(_subscriber) != pair->second.end()) {
                    pair->second.erase(_subscriber);
                    _subscriber->subscribedScopes.erase(fullID);
                    /*find all pieces of info that are affected by this and do the rendez-vous process, only if the RootScope is not a known scope in the range reserved for *-over-ICN abstractions*/
                    known_scope = (known_rootscopes.find(fullID) != known_rootscopes.end())? 1 : 0;
                    switch (known_scope) {
                        case 1:
                            click_chatter("LocalRV: Do not perform Rendevous again after unsubscribe Scope for unicast-dissemination");
                            break;
                        case 0:
                        {
                            InformationItemSet _informationitems;
                            ...
                            for (InformationItemSetIter pub_it = _informationitems.begin(); pub_it != _informationitems.end(); pub_it++) {
                                ...
                                rendezvous(pub_it.get()->_iipointer, subscribers,update_only);
                            }
                        }
                            break;
                        default:
...
unsigned int LocalRV::unsubscribe_info(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only) {
    unsigned int ret;
    unsigned int known_scope;
    InformationItem *pub;
    Scope *fatherScope;
    ...
    if (pub != pubIndex.default_value()) {
        if (fatherScope->strategy == strategy) {
            ...
            if (pair->second.find(_subscriber) != pair->second.end()) {
                ...
                /*do the rendez-vous if there are any left publishers and subscribers, only if the RootScope is not a known scope in the range reserved for *-over-ICN abstractions*/
                pub->getRootScopes(_RootScopeids);
                known_scope = (known_rootscopes.find((*_RootScopeids.begin())._strData) != known_rootscopes.end())? 1 : 0;
                switch (known_scope) {
                    case 1:
                    {
                        click_chatter("LocalRV: Do not perform Rendevous again after unsubscribe Information Item for unicast-dissemination");
                    }
                        break;
                    case 0:
                    {
...
void LocalRV::requestTMAssistanceForRendezvous(InformationItem *pub, RemoteHostSet &_publishers, RemoteHostSet &_subscribers, IdsHashMap &IDs,bool update_only) {
  //click_chatter("requestTMAssistanceForRendezvous, update_only==%d",update_only);
  ...
  /*Publish a request to the TM*/
    ...
    if (update_only)
      request_type = UPDATE_FID;
    else
      request_type = MATCH_PUB_SUBS;
...
/*everything should be sent to the local proxy using the blackadder API*/
void LocalRV::requestTMAssistanceForRendezvous(InformationItem *pub,
                                               RemoteHostSetItem &_publisher,
                                               RemoteHostSet &_subscribers,
                                               IdsHashMap &IDs,
                                               bool update_only
                                               ) {
    //click_chatter("requestTMAssistanceForRendezvous, update_only==%d",update_only);
    ...
    /********FOR THE API*********/
    unsigned char typeForAPI = PUBLISH_DATA;
    unsigned char IDLenForAPI = 3 /*/FF..FE/FF..FD/nodeID*/ * PURSUIT_ID_LEN / PURSUIT_ID_LEN;
    unsigned char strategy = IMPLICIT_RENDEZVOUS;
    /****************************/
    unsigned char request_type;
    if (update_only)
    ...
    unsigned char IDs_total_bytes = 0;
    int ids_index = 0;
    click_chatter("LocalRV: Publish Request for path to the TM under Scope: %s", gc->nodeTMScopeUC.quoted_hex().c_str());
    for (IdsHashMapIter iter = pub->ids.begin(); iter != pub->ids.end(); iter++) {
        IDs_total_bytes += (*iter).first.length();
    }
    /*allocate the packet*/
    packet_len = /*For the blackadder API*/ sizeof (typeForAPI) + sizeof (IDLenForAPI) + 3 * PURSUIT_ID_LEN + sizeof (strategy) + FID_LEN/*END OF API*/\
    /*PAYLOAD*/ + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) /*sizeof(numberOfPubs)*/ + NODEID_LEN /*single publisher is the size of single NODEID_LEN*/ + sizeof (no_subscribers) /*sizeof(numberOfSubs)*/ + _subscribers.size() * NODEID_LEN \
    + sizeof (no_ids) /*sizeof(numberOFIDs)*/ + pub->ids.size() * sizeof (unsigned char) +IDs_total_bytes;
    p = Packet::make(50, NULL, packet_len, 0);
    /*For the API*/
    memcpy(p->data(), &typeForAPI, sizeof (typeForAPI));
    memcpy(p->data() + sizeof (typeForAPI), &IDLenForAPI, sizeof (IDLenForAPI));
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI), gc->nodeTMScopeUC.c_str(), gc->nodeTMScopeUC.length());
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length(), &strategy, sizeof (strategy));
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy), gc->TMFID._data, FID_LEN);
    /*Put the payload*/
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN, &request_type, sizeof (request_type));
    /*put the dissemination strategy of pub*/
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type), &pub->strategy, sizeof (pub->strategy));
    /*put the publisher IDs*/
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy), &no_publishers, sizeof (no_publishers));
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) + publisher_index, _publisher._rhpointer->remoteHostID.c_str(), _publisher._rhpointer->remoteHostID.length());
    publisher_index += _publisher._rhpointer->remoteHostID.length();
    /*put the subscriber IDs*/
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) + publisher_index, &no_subscribers, sizeof (no_subscribers));
    for (RemoteHostSetIter iter = _subscribers.begin(); iter != _subscribers.end(); iter++) {
        memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) + publisher_index + sizeof (no_subscribers) + subscriber_index, (*iter)._rhpointer->remoteHostID.c_str(), (*iter)._rhpointer->remoteHostID.length());
        subscriber_index += (*iter)._rhpointer->remoteHostID.length();
    }
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) + publisher_index + sizeof (no_subscribers) + subscriber_index, &no_ids, sizeof (no_ids));
    /*put the pathIDs of the information item*/
    for (IdsHashMapIter iter = IDs.begin(); iter != IDs.end(); iter++) {
        unsigned char IDLength = (unsigned char) (*iter).first.length() / PURSUIT_ID_LEN;
        memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) + publisher_index + sizeof (no_subscribers) + subscriber_index + sizeof (no_ids) + ids_index, &IDLength, sizeof (IDLength));
        memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (pub->strategy) + sizeof (no_publishers) + publisher_index + sizeof (no_subscribers) + subscriber_index + sizeof (no_ids) + ids_index + sizeof (IDLength), (*iter).first.c_str(), (*iter).first.length());
        ids_index += sizeof (IDLength) + (*iter).first.length();
    }
    p->set_anno_u32(0, RV_ELEMENT);
    output(0).push(p);
...
/*everything should be sent to the local proxy using the blackadder API*/
void LocalRV::requestTMAssistanceForiSubscription(StringSet &_isubscribers,
                                                  bool update_only
                                                  ) {
    /*Publish a request to the TM*/
    int packet_len;
    ...
    /********FOR THE API*********/
    unsigned char typeForAPI = PUBLISH_DATA;
    unsigned char IDLenForAPI = 3 /*/FFFFFFFFFD/nodeID*/ * PURSUIT_ID_LEN / PURSUIT_ID_LEN;
    unsigned char strategy = IMPLICIT_RENDEZVOUS;
    /****************************/
    unsigned char request_type;
    if (update_only)
    ...
    request_type = MATCH_PUB_iSUBS;
    unsigned char no_publishers = 1;
    unsigned char no_subscribers = _isubscribers.size();
    int subscriber_index = 0;
    click_chatter("LocalRV: Publish Request for path to the TM under Scope: %s", gc->nodeTMScopeUC.quoted_hex().c_str());
    /*allocate the packet*/
    packet_len = /*For the blackadder API*/ sizeof (typeForAPI) + sizeof (IDLenForAPI) + IDLenForAPI * PURSUIT_ID_LEN + sizeof (strategy) + FID_LEN/*END OF API*/\
    /*PAYLOAD*/ + sizeof (request_type) + sizeof (no_publishers) /*sizeof(numberOfPubs)*/ + NODEID_LEN /*single publisher is the size of single NODEID_LEN*/ + sizeof (no_subscribers) /*sizeof(numberOfSubs)*/ + no_subscribers * NODEID_LEN;
    p = Packet::make(50, NULL, packet_len, 0);
    /*For the API*/
    memcpy(p->data(), &typeForAPI, sizeof (typeForAPI));
    memcpy(p->data() + sizeof (typeForAPI), &IDLenForAPI, sizeof (IDLenForAPI));
    ...
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN, &request_type, sizeof (request_type));
    /*put the publisher IDs*/
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type), &no_publishers, sizeof (no_publishers));
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (no_publishers), gc->nodeID.c_str() /*Im the publisher of the request*/, gc->nodeID.length());
    /*put the subscriber IDs*/
    memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (no_publishers) + gc->nodeID.length(), &no_subscribers, sizeof (no_subscribers));
    for (StringSetIter it = _isubscribers.begin(); it != _isubscribers.end(); it++) {
        memcpy(p->data() + sizeof (typeForAPI) + sizeof (IDLenForAPI) + gc->nodeTMScopeUC.length() + sizeof (strategy) + FID_LEN + sizeof (request_type) + sizeof (no_publishers) + gc->nodeID.length() + sizeof (no_subscribers) + subscriber_index, (*it)._strData.c_str(), NODEID_LEN);
        subscriber_index += NODEID_LEN;
    }
    p->set_anno_u32(0, RV_ELEMENT);
    output(0).push(p);
}
void LocalRV::requestTMAssistanceForNotifyingSubscribers(unsigned char request_type,
							 StringSet &IDs,
							 RemoteHostSet &_subscribers,
...
void LocalRV::rendezvous(InformationItem *pub,
                         RemoteHostSet &_subscribers,
                         bool update_only
                         ){
    unsigned int known_scope;
  //click_chatter("rendezvous, update_only==%d",update_only);
    /*rootscope identifiers determine whether the request is pure ICN or *-over-ICN*/
    StringSet _RootScopeids;
    pub->getRootScopes(_RootScopeids);
    ...
    if (_publishers.size() > 0) {
        if (pub->strategy == NODE_LOCAL) {
	      ...
	      notifyLocalPublisher(pub, &gc->iLID,update_only);
            } else {
	      notifyLocalPublisher(pub, NULL,update_only);
            }
        } else if (pub->strategy == DOMAIN_LOCAL) {
            /*check if the first root scope is a POINT known scope - multiple all-POINT scopes is a possibility but mutliple scopes of mixed POINT and random scopes is prohibited and the NAP enforce this restriction*/
            known_scope = (known_rootscopes.find((*_RootScopeids.begin())._strData) != known_rootscopes.end())? 1 : 0;
            switch (known_scope) {
                case 1:
                {
                    for (RemoteHostSetIter pub_it = _publishers.begin(); pub_it != _publishers.end(); pub_it++) {
                        /*For POINT, Request TM assistance for each Publisher individually*/
                        click_chatter("LocalRV: Request TM Assistance for Publisher: %s , with InformationItem: %s", (*pub_it)._rhpointer->remoteHostID.c_str(), (*pub->ids.begin()).first.quoted_hex().c_str());
                        requestTMAssistanceForRendezvous(pub, (*pub_it), _subscribers, pub->ids,update_only);
                    }
                }
                    break;
                case 0:
                    requestTMAssistanceForRendezvous(pub, _publishers, _subscribers, pub->ids,update_only);
                    break;
                default:
                    break;
            }
...
void LocalRV::getHTTPgw(RemoteHostSet &_subscribers) {
    InformationItem *pub;
    /*check if the HTTP default InformationItem is already there...*/
    pub = pubIndex.get(gc->HTTP_DEFAULT_IID);
    if (pub != pubIndex.default_value()) {
        /*there is a subscriber or publisher for the HTTP default IID (wildcard name)*/
        pub->getSubscribers(_subscribers);
        return;
    }
    return;
}
...
void LocalRV::notifyLocalPublisher(InformationItem *pub, BABitvector *FID,
				   bool update_only) {
  //click_chatter("notifyLocalPublisher, update_only==%d",update_only);
    WritablePacket *p;
    /********FOR THE API*********/
    unsigned char typeForAPI = PUBLISH_DATA;
