...
public:
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int publish_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int advertise_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int readvertise_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int unpublish_scope(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int unpublish_info(RemoteHost *_publisher, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int subscribe_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int subscribe_root_scope(RemoteHost *_subscriber, String &ID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int subscribe_inner_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int subscribe_info(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @todo should this send true or false for update_only in rendezvous()?
     * 
     * @param _subscriber the RemoteHost that issued the request
     * @param ID a single fragment identifying the Scope in the context of the father Scope. 
     * @param prefixID one or more fragments identifying the father Scope.
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int unsubscribe_scope(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @return one of the return codes (see blackadder_enums.hpp).
     */
    unsigned int unsubscribe_info(RemoteHost *_subscriber, String &ID, String &prefixID, unsigned char &strategy, bool update_only);
     ...
     * @param update_only boolean if true, then the TM will be told to update the FID, but will not send a START_PUBLISH, this would be the case if subscribers have only left. 
     */
  void notifyLocalPublisher(InformationItem *pub, BABitvector *FID,bool update_only);
     ...
     * The topology manager may be required in order to create multicast trees and the respective LIPSIN identifiers. In such case the Topology Manager will notify the publishers directly.
     * 
     ...
     * @param update_only boolean if true, then the TM will be told to update the FID, but will not send a START_PUBLISH, this would be the case if subscribers have only left. 
     */
  void rendezvous(InformationItem *pub, RemoteHostSet &_subscribers, bool update_only);
    /**@brief getHTTPgw method is used to retrieve the set of subscribers for the default HTTP default InformationItem
     *
     * @param _subscribers a reference to a set of subscribers of the HTTP default InformationItem.
     */
    void getHTTPgw(RemoteHostSet &_subscribers);
     ...
     * @param update_only boolean if true, then the TM will be told to update the FID, but will not send a START_PUBLISH, this would be the case if subscribers have only left. 
     */
  void requestTMAssistanceForRendezvous(InformationItem *pub, RemoteHostSet &_publishers, RemoteHostSet &_subscribers, IdsHashMap &IDs, bool update_only);
    /**@brief Using the Blackadder API this method will publish a request (using the IMPLICIT_RENDEZVOUS strategy) for topology formation to the topology manager.
     *
     * This is an overloaded function whereby a single publisher and multiple subscribers are passed to the TM. This is to achieve uni-cast semantics for *-over-ICN 
     * abstractions
     * This publication will contain the type of this request which is MATCH_PUB_SUBS, the set of labels for publishers and subscribers and all Information Identifiers.
     *
     * Information IDs are not required by the TM, but the TM will then use them to notify all publishers to START or STOP publishing data for the InformationItem.
     ...
     *
     * @param pub a pointer to an InformationItem for which rendezvous took place.
     * @param _publisher a reference to a single publisher for which rendezvous took place for the provided InformationItem.
     * @param _subscribers a reference to a set of subscribers for which rendezvous took place for the provided InformationItem.
     * @param IDs the set of identifiers identifying the InformationItem.
     * @param update_only boolean if true, then the TM will be told to update the FID, but will not send a START_PUBLISH, this would be the case if subscribers have only left.
     */
    void requestTMAssistanceForRendezvous(InformationItem *pub, RemoteHostSetItem &_publisher, RemoteHostSet &_subscribers, IdsHashMap &IDs, bool update_only);
    /**@brief Using the Blackadder API this method will publish a request (using the IMPLICIT_RENDEZVOUS strategy) for a unicast path formation towards an implicit subscriber to the topology manager.
     *
     * This publication will contain the type of this request which is MATCH_PUB_iSUBS, the set of labels for the publisher and the subscriber.
     *
     *
     *
     * @param _isubscriber a reference to the implicit subscriber.
     * @param update_only boolean if true, then the TM will be told to update the FID, but will not send a START_PUBLISH, this would be the case if subscribers have only left.
     */
    void requestTMAssistanceForiSubscription(StringSet &_isubscriber, bool update_only);
    ...
    /** @brief A set of Known *-over-ICN scopes
     *
     */
    StringSet known_rootscopes;
    /** @brief A Hash Table of Known namespaces (or root scopes)
     *
     */
    KnownRootHashMap rootScope_namesapce_Index;
};
