...
#include "tomgmnetlink.hh"

CLICK_DECLS

ToMGMNetlink::ToMGMNetlink() {
}

ToMGMNetlink::~ToMGMNetlink() {
    click_chatter("ToMGMNetlink: destroyed!");
}

int ToMGMNetlink::configure(Vector<String> &conf, ErrorHandler */*errh*/) {
    netlink_element = (Netlink *) cp_element(conf[0], this);
    //click_chatter("ToMGMNetlink: configured!");
    return 0;
}

#if CLICK_LINUXMODULE || CLICK_BSDMODULE
int ToMGMNetlink::initialize(ErrorHandler *errh) {
    _task = new Task(this);
...
# endif
    ...
    //click_chatter("ToMGMNetlink: initialized!");
    return 0;
}
#else
int ToMGMNetlink::initialize(ErrorHandler */*errh*/) {
...
void ToMGMNetlink::cleanup(CleanupStage stage) {
...
# endif
    ...
    click_chatter("ToMGMNetlink: Cleaned Up!");
}

void ToMGMNetlink::push(int, Packet *p) {
...
# endif
#else
    nlh->nlmsg_pid = PID_MAPI;
    netlink_element->out_buf_queue.push(final_p);
    add_select(netlink_element->management_fd, SELECT_WRITE);
#endif
}

#if CLICK_LINUXMODULE || CLICK_BSDMODULE

bool ToMGMNetlink::run_task(Task *t) {
    int ret;
...
void ToMGMNetlink::selected(int management_fd, int mask) {
...
# endif
            ...
            msg.msg_iov = iov;
            msg.msg_iovlen = 1;
            sendmsg(management_fd, &msg, MSG_WAITALL);
            /*remove buffer from queue and free it*/
            newPacket->kill();
            netlink_element->out_buf_queue.pop();
            if (netlink_element->out_buf_queue.empty()) {
                remove_select(management_fd, SELECT_WRITE);
            }
        } else {
            remove_select(management_fd, SELECT_WRITE);
            add_select(management_fd, SELECT_READ);
        }
    }
}
#endif
