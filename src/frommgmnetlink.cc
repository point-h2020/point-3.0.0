...
FromMGMNetlink::FromMGMNetlink() {
}

FromMGMNetlink::~FromMGMNetlink() {
    click_chatter("FromMGMNetlink: destroyed!");
}

int FromMGMNetlink::configure(Vector<String> &conf, ErrorHandler */*errh*/) {
    netlink_element = (Netlink *) cp_element(conf[0], this);
    //click_chatter("FromMGMNetlink: configured!");
    return 0;
}

#if CLICK_LINUXMODULE || CLICK_BSDMODULE
int FromMGMNetlink::initialize(ErrorHandler *errh) {
...
# endif
    ...
    //click_chatter("FromMGMNetlink: initialized!");
    return 0;
}
#else
int FromMGMNetlink::initialize(ErrorHandler */*errh*/) {
    add_select(netlink_element->management_fd, SELECT_READ);
    //click_chatter("FromMGMNetlink: initialized!");
    return 0;
}
#endif

void FromMGMNetlink::cleanup(CleanupStage stage) {
...
# endif
    ...
    click_chatter("FromMGMNetlink: Cleaned Up!");
}

#if CLICK_LINUXMODULE || CLICK_BSDMODULE

bool FromMGMNetlink::run_task(Task *t) {
...
void FromMGMNetlink::selected(int fd, int mask) {
...
# ifdef __linux__
        total_buf_size = recv(fd, management_fake_buf, 1, MSG_PEEK | MSG_TRUNC | MSG_WAITALL);
# else
#  ifdef __APPLE__
        socklen_t _option_len = sizeof(total_buf_size);
        if (recv(fd, management_fake_buf, 1, MSG_PEEK | MSG_DONTWAIT) < 0 || getsockopt(fd, SOL_SOCKET, SO_NREAD, &total_buf_size, &_option_len) < 0)
#  else
        if (recv(fd, management_fake_buf, 1, MSG_PEEK | MSG_DONTWAIT) < 0 || ioctl(fd, FIONREAD, &total_buf_size) < 0)
#  endif
        {
