...
#ifndef CLICK_FROMMGMNETLINK_HH
#define CLICK_FROMMGMNETLINK_HH
...
/**
 * @brief (Blackadder Core) The FromMGMNetlink Element receives packets from applications, annotates and pushes them to the LocalProxy Element.
 * 
 ...
 * 
 * FromMGMNetlink uses a kernel mutex when running in kernel space since packets are received in the context of the process and then are processed by a Click Task. 
 * A queue that is secured with the mutex is used for that transition.
 */
class FromMGMNetlink : public Element {
public:
    /**
     ...
     * @return 
     */
    const char *class_name() const {return "FromMGMNetlink";}
    /**
