...
public:
	 ...
	 *
	 * 1) A Packet is pushed by the ToNetlink Element. An application has sent a pub/sub request. See blackadder_enums.hpp for possible cases.
    ...
    /** @brief: store the identifiers of nodes that make implicit subscription
     *
     * @param _isubscriberID a reference to the node identifier of the isubscriber
     * @retrun True if the storage is successful, false if not
     */
    bool storeActiveNode(String &_isubscriberID, Vector<String> &IDs);
	 ...
	 * The default behaviour for START_PUBLISH is to notify a random publisher unless it is the result of an UNSUBSCRIBE_INFO (Note this behaviour has changed since 0.4 when it was only notified in the first case).
	...
	/**@brief  Handles a notification sent by the control reliability module.
	 *
	 * This method is called either to verify a successful delivery or to notify about a failed request transfer.
	 * In both cases the localProxy parses the packet's payload and creates the corresponding event that is later sent to the local app(s) that emitted the request. 
	 */
	void handleControlReliabilityNotification(Packet *p);
    /** @brief Handles a request issued by a Management app (Mapp)
     * This method is called whenever a message is recieved through the Netlink Management socket which means a Mapp has sent a request through MAPI to the core node asking for
     * management information or setting a configuration parameter
     * @param p A Click packet containing the request in a TLV format
     * @param _localhost A pointer to the local process that issued the request
     */
    void handleMappRequest(Packet *p, LocalHost *_localhost);
    /**@brief updates all the states of ICN IDs and NIDs depending on change of the connectivity status of the node.
     * currently used for management notifications by the LinkStateMonitor to inform blackadder of node disconnect/reconnect states (i.e. isolation/reconnection to the network)
     */
    void updateStates(unsigned char &state);
    /* @brief This method is called whenever a NAP sends a PUBLISH_DATA_iMULTICAST request to blackadder.
     * the LocalProxy first finds the respective ActivePublication. It finds the unicast FIDs for the set of implicit subscribers and OR the FIDs to create a single multicast FID to the nodes
     * the method is used as part of facilitating concidental multicast
     * @param ID A reference to the full identifier of the published information item.
     * @param type the publication type: PUBLISH_DATA_iMULTICAST
     * @param p A Click packet containing ONLY some headroom and the DATA to be published.
     * @param isubscriberIDs the set of nodeIDs of implicit subscripers
     */
    void handleUserMulticastRequest(String &ID, unsigned char &type, Packet *p, StringSet &isubscriberIDs);
	/**
     ...
     * @param APItype the API type of the message, used when the message is passed to local subscribers
	 */
    void handleUserPublication(String &ID, Packet *p, LocalHost *_localhost, unsigned char APItype);
    /* @brief This method is called whenever a LocalHost (application or Click Element) sends a publish_data request.
     * overloaded from the previous function to introduce the type field
     * The LocalProxy first finds the respective ActivePublication. It checks if the assigned FID (to subscribers) is zero or the internal Link identifier. In that case there are no remote subscribers.
     * Then it looks for any local subscribers by calling the findLocalSubscribers() method. Note that an ActivePublication may be known with multiple identifiers. That method looks for all of them.
     *
     * Finally, if necessary, it copies the packet and pushes it to all local subscribers and to the network using the stored FID.
     * @param ID A reference to the full identifier of the published information item.
     * @param type the type of the packet sent to remote subscribers
     * @param p A Click packet containing ONLY some headroom and the DATA to be published.
     * @param _localhost The LocalHost sent the request.
     * @param APItype the API type of the message, used when the message is passed to local subscribers
     */
    void handleUserPublication(String &ID, unsigned char &type, Packet *p, LocalHost *_localhost, unsigned char APItype);
     ...
     * @param APItype the API type of the message, used when the message is passed to local subscribers
	 */
	void handleUserPublication(String &ID, BABitvector &FID_to_subscribers, Packet *p, LocalHost *_localhost, unsigned char APItype);
    /* @brief This method is similar to the above. It is called whenever the IMPLICIT_RENDEZVOUS strategy is used.
     * overloaded from the previous function to introduce the type field
     *
     * As in the previous method, it looks for local subscribers. However, it does not look if there is an assigned FID for remote subscribers.
     ...
     * @param APItype the API type of the message, used when the message is passed to local subscribers
     */
    void handleUserPublication(String &ID, unsigned char &type, BABitvector &FID_to_subscribers, Packet *p, LocalHost *_localhost, unsigned char APItype);
    /* @brief This method is called whenever a TM management publication is recieved.
     *
     * As in the previous method, it looks for local subscribers.
     * It processes the TM notification if it is intended for the node, otherwise it push the notification to remote subscribers.
     *
     * @param ID A reference to the full identifier of the published information item.
     * @param type the type of the packet sent to the domain TM, which is a remote host
     * @param FID_to_subscribers A LIPSIN identifier provided by the LocalHost. It will be used to push the publication to the network.
     * @param p A Click packet containing ONLY some headroom and the DATA to be published.
     * @param _localhost The LocalHost sent the request.
     */
    void handleUserTMPublication(String &ID, unsigned char &type, BABitvector &FID_to_subscribers, Packet *p, LocalHost *_localhost);
    /**@breif process TM information for path management
     *
     * @param p A Click packet containing ONLY some headroom and the DATA to be published.
     */
    void processTMNotification(Packet *p);
     ...
     * @param APItype the API type of the message, used when the message is passed to local subscribers
	 */
    void handleNetworkPublication(Vector<String> &IDs, Packet *p, unsigned char &APItype);
    ...
    /** This method is called whenever an implicit subscription request must be published to the local rendezvous point.
     *
     * since this is publication to the local rendezvous, a packet is created according to the exported API (PUBLISHED_DATA_iSUB event) and sent to the LocalRV Element.
     *
     * @param nodeID The node identifier of the implicit subscriber.
     */
    void publishiReqToRV(StringSet &nodeID);
     ...
     * @param APItype the API type of the message, used when the message is passed to local subscribers
	 */
    void pushDataToLocalSubscriber(LocalHost *_localhost, String &ID, Packet *p, unsigned char &APItype);
	/**@brief This method pushes data to remote subscribers either with IMPLICIT_RENDEZVOUS or DOMAIN_LOCAL strategies
	 *
    ...
    /**@brief This method is similar to the previous one but overloaded to introduce the type field
     *
     * The LocalProxy will use the provided FID_to_subscribers to forward the packet to the network.
     *
     * @param IDs a reference to a vector that contains all information identifiers that should be copied in the network publication.
     ...
     * @param p
     */
    void pushDataToRemoteSubscribers(Vector<String> &IDs, unsigned char &type, BABitvector &FID_to_subscribers, Packet *p);
    ...
    /**@brief A HashTable that maps an ActiveNode identifier (NODEID of NODEID_LEN) to a pointer of ActiveNode.
     */
    ActiveNodeMap activeNodeIndex;
