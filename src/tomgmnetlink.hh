...
#ifndef CLICK_ToMGMNetlink_HH
#define CLICK_ToMGMNetlink_HH
...
/**@brief (Blackadder Core) The ToMGMNetlink Element is the Element that sends packets to applications.
 * 
 * The LocalProxy pushes annotated packets to the ToMGMNetlink element, which then sends them to the right applications using the provided packet annotation.
 */
class ToMGMNetlink : public Element {
public:
    /**
     ...
     * @return 
     */
    const char *class_name() const {return "ToMGMNetlink";}
    /**
    ...
    /**@brief the push method is called by the element connected to the ToMGMNetlink element (i.e. the LocalProxy) and pushes a packet.
     * 
