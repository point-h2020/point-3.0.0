...
void InformationItem::getRootScopes(StringSet &_ids) {
    IdsHashMapIter it ;
    String rootscopeid;
    for(it= ids.begin(); it != ids.end();it++) {
        rootscopeid = (*it).first.substring(0, PURSUIT_ID_LEN);
        _ids.find_insert(rootscopeid);
    }
}

unsigned char InformationItem::getRootScopes(KnownRootHashMap *known_root_scopes) {
    unsigned char _ns;
    String rootscopeid;
    IdsHashMapIter it ;
    it = ids.begin();
    rootscopeid = (*it).first.substring(0, PURSUIT_ID_LEN);
    _ns = known_root_scopes->get(rootscopeid);
    if (_ns == known_root_scopes->default_value()) {
        /*Unknown so treat as native ICN namespace*/
