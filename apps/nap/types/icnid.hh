...
public:
    ...
    /*!
     * \brief Creates and returns a pointer to an ID after the ip address
     * parameter passed, which falls under the IGMP data scope.
     *
     * \param ipAddress the ip address to create the id. Leaving this parameter
     * empty or passing NULL simply returns an ID of the IGMP data scope 
     */
    static IcnId* createIGMPDataScopeId(IpAddress* ipAddress = NULL);
    /*!
     * \brief Creates and returns a pointer to an ID after the ip address
     * parameter passed, which falls under the IGMP control scope.
     *
     * \param ipAddress the ip address to create the id. Leaving this parameter
     * empty or passing NULL simply returns an ID of the IGMP control scope 
     */
    static IcnId* createIGMPCtrlScopeId(IpAddress* ipAddress = NULL);
    /*!
     * \brief Creates and returns a pointer to the IGMP control scope ID that 
     * corresponds to the parameter passed data scope id.
     *
     * \param the original data scope id
     */
    static IcnId* createIGMPCtrlFromDataScopeId(IcnId dataId);
    /*!
     * \brief Creates and returns a pointer to the data scope ID that 
     * corresponds to the parameter passed IGMP cotrol scope id.
     *
     * \param the original IGMP control scope id
     */
    static IcnId* createIGMPDataFromCtrlScopeId(IcnId ctrlId);
    /*!
     * \brief Creates and returns the IP address from the parameter passed 
     * IGMP control or data scope id.
     *
     * \param the original IGMP control or data scope id
     */
    static IpAddress extractIPAddrFromCId(IcnId cId);
    /*!
