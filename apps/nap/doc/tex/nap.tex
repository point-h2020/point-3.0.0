...
\subsection{\texttt{delayHttpResponseDelivery} and \texttt{delayHttpResponseTime}}
To help the sNAP to form larger multicast group sizes this option allows the delayed delivery of HTTP responses to the HTTP handler once received by the HTTP proxy from an IP service endpoint. Note, the delivery of succeeding HTTP responses is not affected by this option. If the delayed HTTP response delivery variable is set a default value of 10\,ms is assumed unless explicitly configured with \texttt{delayHttpResponseTime}.

\subsection{\texttt{delayMpdDelivery} and \texttt{delayMpdTime}}
In case the initial delivery of the HTTP response comprising an MPD file for MPEG DASH scenarios the sNAP allows to delay the delivery of HTTP requests which have 'mpd' at the end of the web resource. The \texttt{delayMpdTime} variable allows to configure the time the sNAP delays the delivery of the HTTP request to the web server. The value is in milli seconds. 
...
\subsection{\texttt{igmp\_*}}\label{sec:Introduction_Var_igmp}
The following variables configure specifically the IGMP handler of the \ac{NAP}. Most of the variables control the operation of the handler following the instructions of RFC~2236~\cite{rfc2236}.

\subsubsection{igmp\_disableHandler}
If set to "false", this variable disables the IGMP handler of the \ac{NAP} and IGMP traffic is not specifically managed inside the ICN network. 

\subsubsection{igmp\_genQueryIP}
The variable defines the destination IP address of general queries. According to the RFC~2236 the designated IP is ``224.0.0.1''.

\subsubsection{igmp\_groupLeaveIP}
The variable defines the destination IP address of leave group queries. According to the RFC~2236 the designated IP is ``224.0.0.2''.
...
% Section
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Simplified UML View of NAP Classes}
todo
...
\subsection{PCAP Filters}\label{sec:Code_Demux_Pcap}
TBA
...
\section{IGMP handler}\label{sec:Code_IGMP}

The IGMP handler of the \ac{NAP} bridges transparently two (or more) IP multicast networks that cross the (native multicast) ICN network. It assumes that one content source, who is attached at a individual NAP (sNAP), and multiple content consumers, who are attached at other NAPs (cNAPs), use the IGMP v2 protocol in order to stream video from the sender to multiple consumers. Therefore, the primary role of the IGMP handler is to translate the IGMP v2 messages of the IP networks into ICN messages of the POINT network, thus exploiting the inherent multicast support. The operation of the IGMP handler is similar to \textit{IGMP snooping}, in terms of listening to the IGMP traffic, but differs significantly since it introduces different operations inside the ICN network and converts IP multicast traffic into ICN and vice versa.

In the following, we demonstrate the basic operation of the IGMP handler in the sNAP and cNAP, starting with the bootstrapping procedure which is depicted in Fig.~\ref{fig:igmp_fig1}. The operation is relatively simple, including the publication of the required scopes for the IGMP traffic (control and data plane) by both NAP types and the subscription of the sNAP to the information item of the multicast group that the local server streams. Note that the sNAP must be configured with the multicast groups that it is serving (to be exact, the multicast groups server by IP servers attached to it), in order to know which items it must subscribe to.

\begin{figure}[!h]
	\centering
	\includegraphics[width=1\linewidth]{eps/igmp_fig1}
	\caption{Bootstrapping of the IGMP handler at the cNAP and the sNAP. CID and DID indicate the information scopes identifiers of the IGMP control plane and data plane,respectively. CIDc is the information item identifier of a specific multicast group.}
	\label{fig:igmp_fig1}
\end{figure}

When an IGMP membership report is received at the cNAP, the identifier of the corresponding multicast group will be advertised (Fig.~\ref{fig:igmp_fig2}). The request will be matched with the existing sNAP's subscription and the TM will deliver the forwarding identifier for routing content from the cNAP to the sNAP. Finally, the cNAP will send the membership report of the IP client to the sNAP using implicit rendezvous (publish\_data\_isub), which indicates that the corresponding data stream must be forwarder to the cNAP.

\begin{figure}[!h]
	\centering
	\includegraphics[width=1\linewidth]{eps/igmp_fig2}
	\caption{The operation of a cNAP upon the receipt of a membership report for the multicast group with identifier CIDc.}
	\label{fig:igmp_fig2}
\end{figure}

Then, the sNAP will start pushing the incoming data stream to the cNAP as shown in Fig.~\ref{fig:igmp_fig3}. The operation is again quite simple, as the sNAP pushes the data packet to the publisher-cNAP when the forwarding plane is enabled, or buffers the packet and publishes the related data plane identifier otherwise.

\begin{figure}[!h]
	\centering
	\includegraphics[width=1\linewidth]{eps/igmp_fig3}
	\caption{The operation of serverNAP (sNAP) upon the receipt of a data packet for the multicast group with identifier CIDc.}
	\label{fig:igmp_fig3}
\end{figure}

The handler properly interfaces with upstream and downstream IGMP nodes. Specifically, the sNAP responds to IGMP queries from upstream with the groups active in the POINT network, and the cNAP generates periodic IGMP queries to downstream nodes, handling their membership reports, no reports (implicit leaves) or explicit leave messages.

\subsection{IGMP handler limitations}
